...
(defmodule compiler
    ...
    (defun comp-defun0 (x)
        (let* ((name (elt x 1))
            ...
            (format code0 "(int arglist, int th);")
    ...
    (defun comp-numeric (stream x env args tail name global test clos)
        (cond ((not optimize-enable)
               (format stream "({int res;Fpargpush(")
               (comp stream (elt x 1) env args nil name global test clos)
               (format stream ",th);Fpargpush(")
               (comp stream (elt x 2) env args nil name global test clos)
               (format stream ",th);")
               (cond ((eq (elt x 0) 'eq) (format stream "res=pfast_eq(th);"))
                     ((eq (elt x 0) '=) (format stream "res=pfast_numeqp(th);"))
                     ((eq (elt x 0) '<) (format stream "res=pfast_smallerp(th);"))
                     ((eq (elt x 0) '<=) (format stream "res=pfast_eqsmallerp(th);"))
                     ((eq (elt x 0) '>) (format stream "res=pfast_greaterp(th);"))
                     ((eq (elt x 0) '>=) (format stream "res=pfast_eqgreaterp(th);"))
                     ((eq (elt x 0) '+) (format stream "res=pfast_plus(th);"))
                     ((eq (elt x 0) '-) (format stream "res=pfast_minus(th);"))
                     ((eq (elt x 0) '*) (format stream "res=pfast_mult(th);"))
                     ((eq (elt x 0) 'mod) (format stream "res=pfast_mod(th);")))
               (format stream "res;})"))
        ...
        (let ((n (cdr (assoc (car x) function-arg))))
           ...
           (cond ((> n 0)
                  ...
                  (for ((ls (cdr x) (cdr ls)))
                       ...
                       (comp stream (car ls) env args nil name global test clos)
                       (format stream ",")))
                 (t (format-object stream (conv-name (car x)) nil)
                    (format stream "(")
                    (for ((ls (cdr x) (cdr ls))
                          (m (abs n) (- m 1)) )
                         ((= m 1)
    ...
    (defun comp-funcall1 (stream x env args tail name global test clos)
        (let ((n (cdr (assoc (car x) function-arg))))
           ...
           (for ((ls (cdr x) (cdr ls))
                ...
                (format stream ";~%")
                (cond ((not optimize-enable)
                        (format stream "Fpshelterpush(temp")
                        (format-integer stream m 10)
                        (format stream ",th);~%"))))
           (for ((ls (cdr x) (cdr ls)))
                ((null ls)
