...
(defmodule compiler
    ...
    (defun check-args-count (x)
        (cond ((eq (car x) 'defun)
              ...
              ((eq (car x) 'defdynamic)
               ...
               (setq global-dynamic (cons (elt x 1) global-dynamic))
               (if (atom (elt x 2))
                   (eval x)))
    ...
    (defun comp-dynamic (stream x env args tail name global test clos)
        ...
        (cond ((member (elt x 1) local-dynamic)
               (format stream "Fpfinddyn(Fmakesym(\"")
               (format-object stream (elt x 1) nil)
               (format stream "\"),th)"))
              ((member (elt x 1) global-dynamic)
               (format stream "Fpfinddyn(Fmakesym(\"")
               (format-object stream (elt x 1) nil)
               (format stream "\"),th)"))
              (t (error* "dynamic: illegal variable" x))))
    ...
    (defun comp-set-dynamic (stream x env args tail name global test clos)
        ...
        (let ((symbol (elt x 1))
              (value (elt x 2)) )
           (cond ((member symbol local-dynamic)
                  (format stream "Fpsetdynamic(Fmakesym(\"")
                  (format-object stream symbol nil)
                  (format stream "\"),")
                  (comp stream value env args tail name global test clos)
                  (format stream ",th)"))
                 ((member symbol global-dynamic)
                  (format stream "Fpsetdynamic(Fmakesym(\"")
                  (format-object stream symbol nil)
                  (format stream "\"),")
                  (comp stream value env args tail name global test clos)
                  (format stream ",th)"))
                 (t (error* "set-dynamic: illegal variable" x)))))
