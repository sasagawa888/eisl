    ...
    (defun tokenize2 (ls token res nest)
        (cond ((char= (car ls) #\()
              ...
              ((and (> (length ls) 1) (char= (car ls) #\space) (char= (cadr ls) #\.))
               (if (string= token "")
                   (tokenize2 (cdr ls) "" (cons (create-string 1 (cadr ls)) res) nest)
                   (tokenize2
                    (cdr ls)
                    ""
                    (cons (create-string 1 (cadr ls)) (cons token res))
                    nest)))
              (t (tokenize2 (cdr ls) (string-append token (create-string (car ls))) res nest))))
