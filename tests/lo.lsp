...
(defun push-buttons (xs board :rest n)
    (if (null n)
        ...
        (setq n (car n)) )
    (cond ((zerop xs) board)
          ((logtest xs 1) (push-buttons (ash xs -1)
                                        (logxor (aref pattern n)
                                                board)
                                        (1+ n)))
          (t (push-buttons (ash xs -1) board (1+ n)))))
...
(defun solver-fast (board)
    (dotimes (i 32)
       (let ((xs i)
             ; 押したボタン (ビット)
             (new-board (push-buttons i board)) )
          ; 1 行目を押す (32 通り)
          ;; 1 行ずつライトを消していく
          (dotimes (j 20)
             (print j)
