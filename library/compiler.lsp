...
(defmodule compiler
    ...
    (defun comp (stream x env args tail name global test clos)
        (cond ((and (fixnump x) (not global))
              ...
              ((and (consp x) (consp (car x)) (eq (car (car x)) 'lambda))
               (comp-immediate-lambda stream x env args tail name global test clos))
    ...
    ;; ({int res;int lambdaN(arg1 arg2 ...){res=body1;res=body2...;return(res);
    ;;     res=lambdaN(val1,var2...);res})
    (defun comp-immediate-lambda (stream x env args tail name global test clos)
        (let* ((lam (elt x 0))
               (arg (elt lam 1))
               (body (cdr (cdr lam)))
               (val (cdr x))
               (immediate-name (string-append "lambda" (convert lambda-immediate <string>))))
            (setq lambda-immediate (+ lambda-immediate 1))
            (format stream "({int res;int ~A" immediate-name)
            (gen-arg2 stream arg)
            (format stream "{")
            (for ((body1 body (cdr body1)))
                 ((null body1))
                 (format stream "res=")
                 (comp stream (car body1) (append arg env) args tail name global test clos)
                 (format stream ";"))
            (format stream "return(res);};res=~A(" immediate-name)
            (for ((val1 val (cdr val1)))
                 ((null val1))
                 (comp stream (car val1) env args tail name global test clos)
                 (format stream ","))
            (format stream "th);res;})")))
