...
(defmodule compiler
    ...
    (defun comp-with-open-input-file (stream x env args tail name global test clos)
        (unless (listp (elt x 1)) (error* "with-open-input-file: not list" (elt x 1)))
        (unless (symbolp (elt (elt x 1) 0)) (error* "with-open-input-file: illegal file name" x))
        (format stream "({int res;")
        (format stream "int ")
        (format stream (convert (conv-name (elt (elt x 1) 0)) <string>))
        (format stream " = ")
        (comp stream (list 'open-input-file (elt (elt x 1) 1)) env args nil name global test clos)
        (format stream ";")
        (comp-progn stream 
    ...
    (defun comp-with-open-output-file (stream x env args tail name global test clos)
        (unless (listp (elt x 1)) (error* "with-open-output-file: not list" (elt x 1)))
        (unless (symbolp (elt (elt x 1) 0)) (error* "with-open-output-file: illegal file name" x))
        (format stream "({int res;")
        (format stream "int ")
        (format stream (convert (conv-name (elt (elt x 1) 0)) <string>))
        (format stream " = ")
        (comp stream (list 'open-output-file (elt (elt x 1) 1)) env args nil name global test clos)
        (format stream ";")
        (comp-progn stream 
                   (cons 'progn (append (cdr (cdr x)) (list (list 'close (elt (elt x 1) 0)))) )
                   (cons (elt (elt x 1) 0) env) args tail name global test clos)
    ...
    (defun comp-catch (stream x env args tail name global test clos)
        (if (and (consp (elt x 2)) (eq (car (elt x 2)) 'with-handler))
            (comp-catch-with-handler stream x env args tail name global test clos)
    ...
    ;; when 2nd argument of catch is (with-handler ...)
    ;; catch tag and throw tag must be inside interpreter. 
    ;; For this reason leave to interpreter with eval
    (defun comp-catch-with-handler (stream x env args tail name global test clos)
        (format stream "({int res; res = ")
        (comp stream (list 'eval (list 'quote x)) env args nil name global test clos)
        (format stream ";res;})"))
