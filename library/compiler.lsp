...
(defglobal not-need-res '(labels flet return-from go tagbody while call-next-method the c-lang c-define c-include c-option))
...
(defun compile-file1 (x)
  (let ((option (cond ((eq (self-introduction) 'windows)
                      ...
                      ((eq (self-introduction) 'linux)
                       "gcc -O3 -w -shared -I/home/header -fPIC -o ")))
        (fname (filename x)))
...
(defun compile-file* (x)
  (let ((comp (cond ((eq (self-introduction) 'windows)
                     "gcc -O3 -shared -o ")
                    ((eq (self-introduction) 'linux)
...
(defun check-args-count (x)
  (cond ((eq (car x) 'defun)
         ...
         (setq function-arg (cons (cons (elt x 1)
                                    (count-args (elt x 2)))
                              function-arg)))
        ((eq (car x) 'defmacro)
        ...
        ((eq (car x) 'defglobal)
         (unless (= (length x) 3)
           (error* "defglobal: illegal form" x))
         (unless (symbolp (elt x 1))
           (error: "defglobal: not symbol" (elt x 1)))
         (setq global-variable (cons (elt x 1) global-variable))
         (if (and (not (member (elt x 1) comp-global-var))
                  (atom (elt x 2)))
             (eval x)))
        ((eq (car x) 'defconstant)
         (unless (= (length x) 3)
           (error* "defconstant: illegal form" x))
         (unless (symbolp (elt x 1))
        ...
        ((eq (car x) 'defgeneric)
         ...
         (setq function-arg (cons (cons (elt x 1)
                                        (count-args (elt x 2)))
                                  function-arg))
         (setq generic-name-arg (cons (cons (elt x 1)
                                            (elt x 2))
                                      generic-name-arg))
         (eval x))
        ((eq (car x) 'defmethod)
  ...
  (cond ((and (fixnump x) (not global))
        ...
        ((and (symbolp x) clos) ;;in lambda
         (cond ((eq x nil) (format stream "NIL"))
               ...
               (t (format stream "fast_convert(Fcdr(Fmakesym(\"" )
                  (format stream (convert x <string>))
                  (format stream "\")))"))))
        ((and (symbolp x) (not clos)) ;;not in lambda
         (cond ((eq x nil) (format stream "NIL"))
               ((eq x t) (format stream "T"))
               ((member x env)
        ...
        ((and (consp x) (eq (car x) 'c-include))
         (comp-c-include x))
        ((and (consp x) (eq (car x) 'c-define))
         (comp-c-define x))
        ((and (consp x) (eq (car x) 'c-lang))
         (comp-c-lang x))
        ((and (consp x) (eq (car x) 'c-option))
         (comp-c-option x))
        ...
        ((listp x)
         (comp-funcall stream x env args tail name global test clos))))
...
(defun comp-lambda (x env global)
  (unless (listp (elt x 1))
    (error* "lambda: not list" (elt x 1)))
  (when (null (cdr (cdr x)))
    (error* "lambda: not exist body" x))
...
(defun lambda-name ()
  (let ((name (conv-name (convert
                (string-append
                  (filename file-name-and-ext)
...
(defun find-free-variable1 (x env args)
  (cond ((null x) nil)
        ...
        ((atom x) nil)
        (t (append (find-free-variable1 (car x) env args)
...
(defun varlis-to-lambda-args (x)
  (cond ((null x) nil)
        ...
        ((eq (car x) ':rest) (cdr x))
        ((symbolp (car x))
         (cons (car x) (varlis-to-lambda-args (cdr x))))
        ((consp (car x))
         (cons (car (car x)) (varlis-to-lambda-args (cdr x))))
  ...
  (cond ((null x) t)
        ((null (cdr x))
         (let* ((varbody (get-method-body (car x)))
           ...
           (if (and (method-need-return-p x)
                     (not (equal (last body) '(call-next-method))))
                (format code2 "return(res);" ))
           (format code2 "}~%" )))
        (t
          (let* ((varbody (get-method-body (car x)))
            ...
            (format code2 "}~%" )
            (comp-defgeneric-body (cdr x) (if (= priority 14) t after))))))

(defun method-need-return-p (x)
  (cond ((null (cdr x)) t)
        ((= (get-method-priority (car x)) 11) t)
        ((= (get-method-priority (car x)) 12) nil)
        ((and (= (get-method-priority (car x)) 13)
              (null (cdr x)))
         nil)
        ((and (= (get-method-priority (car x)) 13)
              (method-need-return-p1 (cdr x)))
         nil)
        ((and (= (get-method-priority (car x)) 14)
              (= (get-method-priority (elt x 1)) 14))
         nil)
        (t t)))
...
(defun comp-if (stream x env args tail name global test clos)
  ...
  (cond ((= (length x) 3)
         ...
         (if (not optimize-enable)
             (format stream " != NIL){~%" )
             (format stream "){~%" ))
         (if (and (not (not-need-res-p (elt x 2))) ;;if cond etc ...
                  (not (tailcallp (elt x 2) tail name))) ;;tail recur
             (format stream "res = "))
...
(defun comp-numeric (stream x env args tail name global test clos)
  (cond ((not optimize-enable)
         ...
         (format stream "));")
         (cond ((eq (elt x 0) 'eq)
                (format stream "res=fast_eq();" ))
               ((eq (elt x 0) '=)
                (format stream "res=fast_numeqp();" ))
               ((eq (elt x 0) '<)
                (format stream "res=fast_smallerp();" ))
               ((eq (elt x 0) '<=)
                (format stream "res=fast_eqsmallerp();" ))
               ((eq (elt x 0) '>)
                (format stream "res=fast_greaterp();" ))
               ((eq (elt x 0) '>=)
                (format stream "res=fast_eqgreaterp();" ))
               ((eq (elt x 0) '+)
                (format stream "res=fast_plus();" ))
               ((eq (elt x 0) '-)
                (format stream "res=fast_minus();" ))
               ((eq (elt x 0) '*)
                (format stream "res=fast_mult();" ))
               ((eq (elt x 0) 'mod)
                (format stream "res=fast_mod();" )))
         (format stream "res;})"))
...
(defun comp-funcall (stream x env args tail name global test clos)
  (cond ((and tail (eq (car x) name)) ;;tail call
         (comp-funcall1 stream x env args tail name global test clos))
        ((not (assoc (car x) function-arg)) ;;interpreter
...
(defun comp-subrcall2 (stream x env args tail name global test clos)
  (cond ((null x)
        ...
        ((null (cdr x))
         ...
         (comp stream (car x) env args nil name global test clos)
         (format stream "))" ))
        (t (format stream "Fcons(fast_inverse(" )
           (comp stream (car x) env args nil name global test clos)
           (format stream ")," )
...
(defun comp-labels2 (stream x env args tail name global test clos)
  ...
  (unless (listp (elt x 1))
    (error* "labels: not list" (elt x 1)))
  (when (null (cdr (cdr x)))
    (error* "labels: not exist body" x))
  (let ((local-name (elt x 0))
    ...
    (format code2 "{~%")
    (format code2 "int res;~%")
    (cond ((and (not optimize-enable)(has-tail-recur-p body local-name))  ;;for tail recursive tempn var;
           (gen-arg3 (length args)))
  ...
  (for ((body1 (cdr (cdr x)) (cdr body1)))
       ((null (cdr body1))
        (if (not (tailcallp (car body1) tail name))
           (format stream "res = "))
        (comp stream (car body1)
             (append (mapcar #'car (elt x 1)) env) args tail name global test clos)
        (if (not (not-need-colon-p (car body1)))
            (format stream ";~%"))
        (format stream "res;})~%"))
       (comp stream (car body1)
...
(defun comp-let* (stream x env args tail name global test clos)
  (unless (listp (elt x 1))
    (error* "let*: not list" (elt x 1)))
  (format stream "({int res;" )
  (comp-let1 stream (elt x 1) (append (mapcar #'car (elt x 1)) env) args tail name global test clos)
  (for ((body1 (cdr (cdr x)) (cdr body1)))
       ((null (cdr body1))
        (if (not (tailcallp (car body1) tail name))
            (format stream "res = "))
...
(defun comp-case1 (stream x env args tail name global test clos key)
  (cond ((null x)
        ...
        ((eq (car (car x)) t)
         ...
         (format stream ";res;})"))
        (t (format stream "else if(Fmember(fast_inverse(" )
...
(defun comp-case-using (stream x env args tail name global test clos)
  ...
  (comp-case-using1 stream (cdr (cdr (cdr (cdr x)))) env args tail name global test clos (elt x 2) (elt x 1)))
...
(defun comp-case-using1 (stream x env args tail name global test clos key pred)
  (cond ((null x)
        ...
        ((eq (car (car x)) t)
         ...
         (comp-cond2 stream (cdr (car x)) env args nil name global test clos)
         (format stream ";res;})"))
        (t (format stream "else if(Fmember1(fast_inverse(" )
           (comp stream key env args nil name global test clos)
           (format stream "),")
           (list-to-c1 stream (car (car x)))
           (format stream ",")
...
(defun has-tail-recur-p1 (x name)
  (cond ((null x) nil)
        ...
        ((eq (car x) name) t)
        ((eq (car x) 'cond)
         (any (lambda (y) (has-tail-recur-p (cdr y) name))
              (cdr x)))
        ((eq (car x) 'if)
         (or (has-tail-recur-p1 (elt x 2) name)
             (and (= (length x) 4)
                  (has-tail-recur-p1 (elt x 3) name))))
        ((eq (car x) 'let)
         (has-tail-recur-p (cdr (cdr x)) name))
        ((eq (car x) 'let*)
         (has-tail-recur-p (cdr (cdr x)) name))
        ((eq (car x) 'dynamic-let)
         (has-tail-recur-p (cdr (cdr x)) name))
        ((eq (car x) 'progn)
         (has-tail-recur-p (cdr x) name))
        ((eq (car x) 'block)
         (has-tail-recur-p (cdr (cdr x)) name))
        (t nil)))
...
(defun comp-convert (stream x env args tail name global test clos)
  (unless (symbolp (elt x 2))
    (error* "convert: not symbol" x))
  (unless (= (length x) 3)
    (error* "convert: illegal form" x))
...
(defun comp-catch (stream x env args tail name global test clos)
  (let ((tag (elt (elt x 1) 1)))
    ...
    (format stream "ret = 0;~%" )
    (cond (unwind-thunk
           (format-object stream unwind-thunk nil)
           (format stream "();" )))
    ...
    (cond ((and (consp form) (eq (car form) 'aref))
          ...
          ((and (consp form) (eq (car form) 'dynamic))
           (let ((newform (list 'set-dynamic (elt form 1) val)))
             (comp stream newform env args tail name global test clos)))
          ((and (consp form) (= (length form) 2) (symbolp (elt form 0)) (symbolp (elt form 1)))
           (let ((newform (list 'set-slot-value val (elt form 1) (list 'quote (eval (list (elt form 0) nil))))))
             (comp stream newform env args tail name global test clos)))
          ((symbolp form)
...
(defun comp-length (stream x env args tail name global test clos)
  (unless (= (length x) 2)
    (error* "length: illegal form" x))
  (unless (or (symbolp (elt x 1))(listp (elt x 1))(general-vector-p (elt x 1)))
...
(defun comp-defdynamic (x)
  (unless (= (length x) 3)
    (error* "defdynamic: illegal form" x))
  (unless (symbolp (elt x 1))
...
(defun comp-set-dynamic (stream x env args tail name global test clos)
  (unless (= (length x) 3)
    (error* "set-dynamic: illegal form" x))
  (unless (symbolp (elt x 1))
...
(defun comp-defmethod (x)
  (cond ((or (eq (elt x 1) 'create)
             (eq (elt x 1) 'initialize-object))
         (format code4 "Feval(" )
         (list-to-c1 code4 x)
         (format code4 ");~%" ))
        (t (let* ((name (elt x 1))
                  (arg (if (listp (elt x 2)) (elt x 2) (elt x 3)))
                  (res (assoc name generic-name-arg)))
             (when (null res)
               (error* "not exist defgeneric " name))
             (unless (has-same-varlis-p arg (cdr res))
...
(defun has-same-varlis-p (x y)
  (cond ((and (null x)(null y))
        ...
        ((and (consp (car x))
              (consp (car y))
              (eq (elt (car x) 0)(elt (car y) 0)))
         (has-same-varlis-p (cdr x)(cdr y)))
        ((and (consp (car x))
              (symbolp (car y))
              (eq (elt (car x) 0)(car y)))
         (has-same-varlis-p (cdr x)(cdr y)))
        ((and (symbolp (car x))
              (consp (car y))
              (eq (car x)(elt (car y) 0)))
         (has-same-varlis-p (cdr x)(cdr y)))
        (t nil)))
...
(defun conv-name1 (ls)
  (cond ((char= (car ls) #\0)
         (string-append "zero" (conv-name2 (cdr ls))))
        ((char= (car ls) #\1)
         (string-append "one" (conv-name2 (cdr ls))))
        ((char= (car ls) #\2)
         (string-append "two" (conv-name2 (cdr ls))))
        ((char= (car ls) #\3)
         (string-append "three" (conv-name2 (cdr ls))))
        ((char= (car ls) #\4)
         (string-append "four" (conv-name2 (cdr ls))))
        ((char= (car ls) #\5)
         (string-append "five" (conv-name2 (cdr ls))))
        ((char= (car ls) #\6)
         (string-append "six" (conv-name2 (cdr ls))))
        ((char= (car ls) #\7)
         (string-append "seven" (conv-name2 (cdr ls))))
        ((char= (car ls) #\8)
         (string-append "eight" (conv-name2 (cdr ls))))
        ((char= (car ls) #\9)
         (string-append "nine" (conv-name2 (cdr ls))))
        (t (conv-name2 ls))))

(defun conv-name2 (ls)
  (cond ((null ls) "")
        ((char= (car ls) #\-)
         (string-append "_" (conv-name2 (cdr ls))))
        ((char= (car ls) #\+)
         (string-append "plus" (conv-name2 (cdr ls))))
        ((char= (car ls) #\*)
         (string-append "star" (conv-name2 (cdr ls))))
        ((char= (car ls) #\/)
         (string-append "slash" (conv-name2 (cdr ls))))
        ((char= (car ls) #\!)
         (string-append "exclamation" (conv-name2 (cdr ls))))
        ((char= (car ls) #\%)
         (string-append "percent" (conv-name2 (cdr ls))))
        ((char= (car ls) #\$)
         (string-append "dollar" (conv-name2 (cdr ls))))
        ((char= (car ls) #\&)
         (string-append "and" (conv-name2 (cdr ls))))
        ((char= (car ls) #\=)
         (string-append "equal" (conv-name2 (cdr ls))))
        ((char= (car ls) #\^)
         (string-append "hat" (conv-name2 (cdr ls))))
        ((char= (car ls) #\~)
         (string-append "tilde" (conv-name2 (cdr ls))))
        ((char= (car ls) #\\)
         (string-append "yen" (conv-name2 (cdr ls))))
        ((char= (car ls) #\|)
         (string-append "vertical" (conv-name2 (cdr ls))))
        ((char= (car ls) #\@)
         (string-append "atmark" (conv-name2 (cdr ls))))
        ((char= (car ls) #\#)
         (string-append "sharp" (conv-name1 (cdr ls))))
        ((char= (car ls) #\:)
         (string-append "colon" (conv-name2 (cdr ls))))
        ((char= (car ls) #\>)
         (string-append "greater" (conv-name2 (cdr ls))))
        ((char= (car ls) #\<)
         (string-append "smaller" (conv-name2 (cdr ls))))
        ((char= (car ls) #\[)
         (string-append "lbracket" (conv-name2 (cdr ls))))
        ((char= (car ls) #\])
         (string-append "rbracket" (conv-name2 (cdr ls))))
        ((char= (car ls) #\{)
         (string-append "lcurl" (conv-name2 (cdr ls))))
        ((char= (car ls) #\})
...
(defun quasi-transfer (x n)
  (cond ((null x) nil)
        ((atom x) (list 'quote x))
        ((and (consp x)(eq (car x) 'unquote)(= n 0))
         (elt x 1))
        ((and (consp x)(eq (car x) 'unquote-splicing)(= n 0))
         (elt x 1))
        ((and (consp x)(eq (car x) 'quasi-quote))
         (list 'list
        ...
        ((and (consp x)(consp (car x))(eq (car (car x)) 'unquote-splicing))
         (list 'cons
               (list 'list (list 'quote 'unquote-splicing)(quasi-transfer (elt (car x) 1) (- n 1)))
               (quasi-transfer (cdr x) n)))
        (t
          (list 'cons
...
(defun optimize-p1 (x)
  (cond ((null x) t)
        ((and (member (elt (car x) 1)
                      (list (class <fixnum>)(class <float>)))
              (subsetp (elt (car x) 2)
                       (list (class <fixnum>)(class <float>))))
         (optimize-p1 (cdr x)))
        (t nil)))
...
(defglobal file-name-and-ext nil)
(defglobal instream nil)
(defglobal type-function nil) ;; for global defun
(defglobal local-type-function nil) ;;for lavels flet syntax
...
(defun inference-file (x)
  ...
  (let ((sexp nil))
    (while (setq sexp (read instream nil nil))
           (cond ((and (consp sexp)(eq (car sexp) 'defun))
  ...
  (let ((labels-func (elt x 1))
    ...
    (while(not(null labels-func))
          (let* ((func (car labels-func))
                 ...
                 (init-type-input (create-list (length arg) (class <object>)))
                 (init-env (create-init-env arg)))
            (setq local-type-function (cons (list name
                                                  (class <object>)
                                                  init-type-input)
                                            local-type-function))
            (setq local-type-env
  ...
  (cond ((and (symbolp x)(eq x t)) type-env)
        ...
        ((and (consp x)(eq (car x) 'setq))
         (cond ((not (symbolp (elt x 1)))
                (warning "setq type mismatch" (elt x 1))
                type-env)
               (t (cons (cons (elt x 1)
        ...
        ((and (consp x)(eq (car x) 'let*))
         (inference-let x type-env))
        ((and (consp x)(eq (car x) 'for))
         (inference-for x type-env))
        ((and (consp x)(eq (car x) 'while))
         (inference-while x type-env))
        ((and (consp x)(eq (car x) 'labels))
         (inference-labels x type-env))
        ((and (consp x)(eq (car x) 'flet))
         (inference-labels x type-env))
        ((and (consp x)(eq (car x) 'function))
         (inference-function x type-env))
        ((and (consp x)(macrop x))
         (inference (macroexpand-1 x) type-env))
  ...
  (cond ((null x) type-env)
        ...
        ((and (consp (car x))(subrp (car (car x))))
         (let ((type-subr (property (car (car x)) 'inference)))
           (block exit-all
             (for ((type type-subr (cdr type)))
                  ((null type) (warning "subr type mismatch" (car x)) 'no)
...
(defun inference-case1 (x type-env)
  (cond ((null x) type-env)
        (t (let ((new-env (inference-case2 (cdr (car x)) type-env)))
             (if (not (eq new-env 'no))
  ...
  (block exit-arg
    (for ((arg x (cdr arg))
          (type y (if (and (>= (length type) 2)
                           (eq (elt type 1) 'repeat)) ;e.g. funcall
                      type
                      (cdr type))))
         ((or (null arg)
              (null type)) (cond ((and (null arg)
                                       (null type)) type-env)
                                 ((and (null arg)
                                       (and (>= (length type) 2)
                                            (eq (elt type 1) 'repeat))) type-env)
                                 (t 'no)))
         (cond ((atom (car arg))
...
(defun find-class (x type-env)
  (cond ((null x) (class <null>))
        ...
        ((and (consp x) (eq (car x) 'labels))
         (find-class (last (elt x 2)) type-env))
        ((and (consp x) (eq (car x) 'flet))
         (find-class (last (elt x 2)) type-env))
        ((and (consp x) (eq (car x) 'cond))
         (find-class-cond (cdr x) type-env))
        ((and (consp x) (eq (car x) 'if))
         (find-class-if x type-env))
        ((and (consp x) (eq (car x) 'quote))
         (class-of (elt x 1)))
...
(assert set-aref (class <null>) (class <object>) (class <general-array*>)(class <integer>)(class <integer>))
