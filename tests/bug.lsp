...
(defun pcoefadd (e c x) 
   (format (standard-output) "pcoefadd ~A ~A ~A ~%" e c x)
   (if (pzerop c)
       x
       (cons e (cons c x))))

(defun pcplus (c p)
   (format (standard-output) "pcplus ~A ~A ~%" c p)
   (if (pcoefp p)
       (cplus p c)
       (psimp (car p) (pcplus1 c (cdr p)))))

(defun pcplus1 (c x)
   (format (standard-output) "pclus1 ~A ~A ~%" c x)
   (cond ((null x)
          (if (pzerop c)
              nil
              (cons 0 (cons c nil))))
...
(defun pctimes (c p) 
   (format (standard-output) "pctimes ~A ~A ~%" c p)
   (if (pcoefp p)
       (ctimes c p)
       (psimp (car p) (pctimes1 c (cdr p)))))

(defun pctimes1 (c x)
   (format (standard-output) "pctimes1 ~A ~A ~%" c x)
   (if (null x)
       nil
       (pcoefadd (car x)
                 (ptimes c (car (cdr x)))
...
(defun pplus (x y) 
   (format (standard-output) "pplus ~A ~A ~%" x y)
   (cond
         ((pcoefp x)
          (pcplus x y))
         ((pcoefp y)
...
(defun pplus1 (x y)
   (format (standard-output) "pplus1 ~A ~A ~%" x y)
   (cond
         ((null x) y)
         ((null y) x)
         ((= (car x) (car y))
...
(defun psimp (var x)
   ...
   (cond
         ...
         ((not (consp x)) x)
         ((pzerop (car x))
          (print "asdf") (car (cdr x)))
         (t
          (cons var x))))

(defun ptimes (x y) 
   (format (standard-output) "ptimes ~A ~A ~%" x y)
   (cond
...
(defun ptimes1 (x y)
   (format (standard-output) "ptimes1 ~A ~A ~%" x y)
   (dynamic-let ((*x* x))
...
(defun ptimes2 (y)
   (format (standard-output) "ptimes2 ~A ~%" y)
   (if (null y)
       nil
...
(defun ptimes3 (y) 
   (format (standard-output) "ptimes3 ~A ~%" y)
   (let ((e ())
         (u ())
         (c ()))
        (tagbody
...
(defun pexptsq (p x)
   (format (standard-output) "pexptsq ~A ~A ~%" p x)
