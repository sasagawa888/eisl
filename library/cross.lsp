...
(defmodule cross
    ...
    (defun analize-defun (x fun)
        (cond ((null x) t)
              (t (analize-sexp (car x) fun)
                 (analize-defun (cdr x) fun))))

    (defun analize-defmodule (x)
        (cond ((null x) t)
              (t (analize (car x))
                 (analize-defmodule (cdr x)))))

    (defun analize-sexp (x fun)
        (cond ((null x) t)
              ((and (atom x)
                    (assoc x globalvars)) (add-global-ref fun x))
              ((and (atom x)
                    (assoc x dynamicvars)) (add-dynamic-ref fun x))
              ((atom x) t)
              ...
              ((eq (car x) 'for) (analize-progn (cdr (cdr (cdr x)))
                                                fun))
              ((eq (car x) 'labels) t)
              ((eq (car x) 'flet) t)
              ...
              ((eq (car x) 'import) t)
              ((macrop (car x)) (analize-sexp (macroexpand-all x)
                                              fun))
              (t (add-fun-ref (car x) fun)
                 (analize-args (cdr x) fun))))
    ...
    (defun analize-if (x fun)
        (cond ((= (length x) 3)
              ...
              (t (analize-sexp (elt x 0) fun)
                 (analize-sexp (elt x 1) fun))))

    (defun analize-cond (x fun)
        (cond ((null x) t)
              (t (analize-args (car x) fun)
                 (analize-cond (cdr x) fun))))
    ...
    (defun reg-fun (x)
        (if (assoc x functions)
            t
            (setq functions
                  (cons (cons x nil) functions))))

    (defun reg-fun1 (x y)
        (setq functions
              (cons (list x y) functions)))

    (defun reg-global (x)
        (if (member x globalvars)
            t
            (setq globalvars
                  (cons (cons x nil) globalvars))))

    (defun reg-dynamic (x)
        (if (member x dynamicvars)
            t
            (setq dynamicvars
                  (cons (cons x nil) dynamicvars))))
