...
(defmodule compiler
    ...
    (defun comp-call-next-method (args)
        (let* ((varbody (get-method-body (car rest-method)))
               (varlis (alpha-conv-varlis (car varbody) args))
               (body (alpha-conv-method (cdr varbody) (method-varlis-to-substlist (car varbody) args)))
               (priority (get-method-priority (car rest-method))) )
            (format code2 "if(")
            (comp-defgeneric-qualifier-cond varlis)
            (format code2 ")~%{")
        ...
        (cond ((null rest-method) t)
              ...
              ((equal (car body) '(call-next-method))
               ...
               (if (not (= priority primary))
                   (progn (setq rest-method (cdr rest-method))
                          (comp-call-next-method args)))
               (comp-call-next-method1 priority (cdr body) env args))
              ;; (if (next-method-p)(call-next-method))
              ((equal (car body) '(if (next-method-p)(call-next-method)))
               ;; if next-method is primary or not exist nest-method, ignore
               (if (and (not (= priority primary)) (not (null (cdr rest-method))))
                   (progn (setq rest-method (cdr rest-method))
                          (comp-call-next-method args)))
               (comp-call-next-method1 priority (cdr body) env args))
              (t
