    ...
    (cond ((null x) t)
          ((eql (car x) ':)
           (let ((y (slice-code (cdr x) 'semicolon)))
              (define-word (elt y 0))
              (interpret-all (elt y 1))))
          ((eql (car x) 'see) (see (elt x 1)) (interpret-all (cdr (cdr x))))
          ((eql (car x) 'do)
          ...
          ((eql (car x) 'if)
           (let* ((y (slice-code (cdr x) 'else))
               ...
               (if* (elt y 0) (elt z 0))
               (interpret-all (elt z 1))))
          (t (interpret (car x)) (interpret-all (cdr x)))))
...
;; word over
(defun over ()
    (if (< (length *data-stack*) 2)
        (error* "not enough data" 'rot) )
...
(defun emit ()
    (let ((data (pop)))
       (if (not (integerp data))
           (error* "not integer" data) )
...
(defun gettoken1 ()
    (let ((token nil))
       (skip-space)
       (skip-paren)
       (while (and (not (null *buffer*)) (not (char= (car *buffer*) #\space)))
...
(defun gettoken3 ()
    (let ((token nil))
       (skip-space)
       (while (and (not (null *buffer*)) (not (char= (car *buffer*) #\")))
          (setq token (cons (car *buffer*) token))
          (setq *buffer* (cdr *buffer*)))
       (setq *buffer* (cdr *buffer*))
       ;;discard "
...
;;skip space
(defun skip-space ()
    (while (and (not (null *buffer*)) (char= (car *buffer*) #\space))
       (setq *buffer* (cdr *buffer*))) )
...
(defun float-list-p1 (ls)
    (cond ((null ls) nil)
          ((char= (car ls) #\.) (integer-list-p (cdr ls)))
