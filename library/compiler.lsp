...
(defmodule compiler
    ...
    (defun comp-defgeneric-body1 (x methods env args)
        (cond ((null x) t)
              ...
              ((equal (car x) '(call-next-method))
               ...
               (comp-call-next-method (eisl-get-method-priority (car rest-method)) args)
    ...
    (defun comp-call-next-method-subr (stream x env args tail name global test clos)
        (format stream "({int res;")
        ;;I write a new one without using the existing code. 
      ...
      (cond ((null rest-method) t)
            (t
              (let* ((varbody (eisl-get-method-body (car rest-method)))
                 ...
                 (comp-defgeneric-qualifier-cond varlis)
                 (format code2 ")~%{")
                 (comp-call-next-method1 body (varlis-to-lambda-args varlis) args)
                 (format code2 "}~%")
                 ;; if next-method is primary then end, else generate rest-methods
                 (if (and next-priority (not (= next-priority primary)) (not (null rest-method)))
    ...
    (defun comp-call-next-method1 (body env args)
        (cond ((null body) t)
              ...
              ;; (if (next-method-p)(call-next-method))
              ((equal (car body) '(if (next-method-p)(call-next-method)))
               ;; if not exist nest-method, ignore
               (if (not (null (cdr rest-method)))
                   (progn (setq rest-method (cdr rest-method))
                          (comp-call-next-method (eisl-get-method-priority (car rest-method)) args)))
               (comp-call-next-method1 (cdr body) env args))
              (t
               (format code2 "res = ")
               (comp code2 (car body) env args nil nil nil nil nil)
