...
res = ({int res=NIL;
if((fast_convert(Fcallsubr(Fcar(Fmakesym("CONSP")),Flist1(fast_inverse(TERM)))) && fast_eq(fast_convert(fast_convert(fast_car(TERM))),fast_convert(Fmakesym("EQUAL"))) && fast_convert(Fcallsubr(Fcar(Fmakesym("CONSP")),Flist1(fast_inverse(fast_convert(fast_car(fast_convert(fast_cdr(TERM))))))))) != NIL){
res = ({int res;
 res=fast_convert(Fcallsubr(Fcar(Fmakesym("SET-PROPERTY")),Fcons(fast_inverse(Fcons(fast_inverse(TERM),fast_inverse(fast_convert(Fcallsubr(Fcar(Fmakesym("PROPERTY")),Fcons(fast_inverse(fast_convert(fast_car(fast_convert(fast_car(fast_convert(fast_cdr(TERM))))))),Flist1(fast_inverse(Fmakesym("LEMMAS"))))))))),Fcons(fast_inverse(fast_convert(fast_car(fast_convert(fast_car(fast_convert(fast_cdr(TERM))))))),Flist1(fast_inverse(Fmakesym("LEMMAS")))))));res;});}
else{
res = ({int res;
 res=fast_convert(Fcallsubr(Fcar(Fmakesym("ERROR")),Fcons(fast_inverse(Fmakestr("%ADD-LEMMA did not like the term: ~a~%")),Flist1(fast_inverse(TERM)))));res;});}
;res;});
if(CELLRANGE(TERM)) Fshelterpop();
return(res);}
...
res = ({int res=NIL;
if(({int res;
 res=fast_convert(Fcallsubr(Fcar(Fmakesym("NULL")),Flist1(fast_inverse(LST))));res;}) != NIL){
res = T;}
else{
res = ADD_LEMMA(fast_convert(fast_car(LST)));
{
temp1 = fast_convert(fast_cdr(LST));
if(CELLRANGE(LST)) Fshelterpop();
LST = temp1;
...
res = ({int res=NIL;
if(fast_not(({int res;
 res=fast_convert(Fcallsubr(Fcar(Fmakesym("CONSP")),Flist1(fast_inverse(TERM))));res;})) != NIL){
res = ({int res=NIL;
if(({int res;
 res = Fset_cdr(Fmakesym("TEMP-TEMP"),fast_inverse(fast_convert(Fcallsubr(Fcar(Fmakesym("ASSOC")),Fcons(fast_inverse(TERM),Flist1(fast_inverse(ALIST)))))));res;}) != NIL){
...
if(Ffreecell() < 900) Fgbc();
res = ({int res;
if((res=fast_convert(Fcallsubr(Fcar(Fmakesym("EQUAL")),Fcons(fast_inverse(X),Flist1(fast_inverse(Fcons(Fmakesym("F"),NIL))))))) == NIL)
if((res=fast_convert(Fcallsubr(Fcar(Fmakesym("MEMBER")),Fcons(fast_inverse(X),Flist1(fast_inverse(LST)))))) !=NIL)
res=res;
else res=NIL;
else res=res;res;})
;
...
if(Ffreecell() < 900) Fgbc();
res = ({int res;
if((res=fast_convert(Fcallsubr(Fcar(Fmakesym("EQUAL")),Fcons(fast_inverse(X),Flist1(fast_inverse(Fcons(T,NIL))))))) == NIL)
if((res=fast_convert(Fcallsubr(Fcar(Fmakesym("MEMBER")),Fcons(fast_inverse(X),Flist1(fast_inverse(LST)))))) !=NIL)
res=res;
else res=NIL;
else res=res;res;})
...
if(CELLRANGE(TERM2)) Fshelterpop();
if(CELLRANGE(TERM1)) Fshelterpop();
return(res);}
...
res = ({int res=NIL;
if(fast_not(({int res;
 res=fast_convert(Fcallsubr(Fcar(Fmakesym("CONSP")),Flist1(fast_inverse(TERM2))));res;})) != NIL){
res = ({int res=NIL;
if(({int res;
 res = Fset_cdr(Fmakesym("TEMP-TEMP"),fast_inverse(fast_convert(Fcallsubr(Fcar(Fmakesym("ASSOC")),Fcons(fast_inverse(TERM2),Flist1(fast_inverse(fast_convert(Fcdr(Fmakesym("UNIFY-SUBST"))))))))));res;}) != NIL){
res = ({int res;
 res=fast_convert(Fcallsubr(Fcar(Fmakesym("EQUAL")),Fcons(fast_inverse(TERM1),Flist1(fast_inverse(fast_convert(fast_cdr(fast_convert(Fcdr(Fmakesym("TEMP-TEMP"))))))))));res;});}
else{
res = ({int res;
...
res = T;}
;res;});}
else if(fast_not(fast_convert(Fcallsubr(Fcar(Fmakesym("CONSP")),Flist1(fast_inverse(TERM1))))) != NIL){
res = NIL;}
else if(fast_eq(fast_convert(fast_convert(fast_car(TERM1))),fast_convert(fast_convert(fast_car(TERM2)))) != NIL){
...
res = ({int res=NIL;
if(({int res;
 res=fast_convert(Fcallsubr(Fcar(Fmakesym("NULL")),Flist1(fast_inverse(LST1))));res;}) != NIL){
res = T;}
...
if(CELLRANGE(LST1)) Fshelterpop();
if(CELLRANGE(LST2)) Fshelterpop();
LST1 = temp1;
LST2 = temp2;
...
if(({int res;
 res=fast_convert(Fcallsubr(Fcar(Fmakesym("CONSP")),Flist1(fast_inverse(TERM))));res;}) != NIL){
res = REWRITE_WITH_LEMMAS(Fcons(fast_inverse(fast_convert(fast_car(TERM))),fast_inverse(REWRITE_ARGS(fast_convert(fast_cdr(TERM))))),fast_convert(Fcallsubr(Fcar(Fmakesym("PROPERTY")),Fcons(fast_inverse(fast_convert(fast_car(TERM))),Flist1(fast_inverse(Fmakesym("LEMMAS")))))));}
else{
res = TERM;}res;})
;
if(CELLRANGE(TERM)) Fshelterpop();
...
res = REWRITE(APPLY_SUBST(fast_convert(Fcdr(Fmakesym("UNIFY-SUBST"))),fast_convert(Fcallsubr(Fcar(Fmakesym("ELT")),Fcons(fast_inverse(fast_convert(fast_car(LST))),Flist1(fast_inverse(fast_immediate(2))))))));}
else{
{
temp1 = TERM;
...
temp3 = FALSE_LST;
if(CELLRANGE(X)) Fshelterpop();
if(CELLRANGE(TRUE_LST)) Fshelterpop();
if(CELLRANGE(FALSE_LST)) Fshelterpop();
X = temp1;
TRUE_LST = temp2;
FALSE_LST = temp3;
goto TAUTOLOGYPloop;};}
...
{
temp1 = fast_convert(Fcallsubr(Fcar(Fmakesym("ELT")),Fcons(fast_inverse(X),Flist1(fast_inverse(fast_immediate(3))))));
temp2 = TRUE_LST;
temp3 = FALSE_LST;
if(CELLRANGE(X)) Fshelterpop();
if(CELLRANGE(TRUE_LST)) Fshelterpop();
...
res = ({int res;
if((res = TAUTOLOGYP(fast_convert(Fcallsubr(Fcar(Fmakesym("ELT")),Fcons(fast_inverse(X),Flist1(fast_inverse(fast_immediate(2)))))),Fcons(fast_inverse(fast_convert(fast_car(fast_convert(fast_cdr(X))))),fast_inverse(TRUE_LST)),FALSE_LST)) != NIL)
if((res=TAUTOLOGYP(fast_convert(Fcallsubr(Fcar(Fmakesym("ELT")),Fcons(fast_inverse(X),Flist1(fast_inverse(fast_immediate(3)))))),TRUE_LST,Fcons(fast_inverse(fast_convert(fast_car(fast_convert(fast_cdr(X))))),fast_inverse(FALSE_LST)))) !=NIL)
res=res;
else res=NIL;
else res=NIL;res;})
;}
