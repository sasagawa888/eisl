...
f_mult (int arglist)
{
  ...
  while (!(IS_NIL (arglist)))
	...
	error (FLT_OVERF, "*", arg);
	  ...
	  && val > 1.0 && fabs (GET_FLT (exact_to_inexact (arg))) >= DBL_MAX)
	error (FLT_OVERF, "*", arg);
	...
	error (FLT_OVERF, "*", arg);
	  ...
	  && val > 1.0 && fabs (GET_FLT (exact_to_inexact (res))) >= DBL_MAX)
	error (FLT_OVERF, "*", arg);
...
f_create_string_input_stream (int arglist)
{
  ...
  EXCEPT (Mem_Failed) error (MALLOC_OVERF, "create-string-input-stream", NIL);
  END_TRY;
  return (res);
}
