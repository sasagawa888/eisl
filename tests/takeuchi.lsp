...
第一要素の位置に合わせる。
（６）cond節におけるショートコメントは各条件帰結の前にあるものと仮定する。
（７）cond節の条件帰結が１行で書けるとしても、総文字数が一定数を超える場合には条件式と帰結式を分けて書く。
（８）クオート、バッククオートのときで１行で書けないときは１文字下げて改行する。
（９）cond節など書法の慣習が定着しているシンタックスはそれに従う。
|#
