...
(defmodule compiler
              ...
              (t
               (let* ((varbody (eisl-get-method-body (car x)))
                   ...
                   (cond ((= priority primary)
                          (comp-defgeneric-primary-cond varlis code2))
                         (t
    ...
    (defun comp-defgeneric-primary-cond (x stream)
        (cond ((null x) t)
              ((eq (car x) ':rest) t)
              ((eq (car x) '%rest) t)
              ;;when method has only one symbol argument, require "1"
              ((symbolp (car x))
               (format stream "1") 
               (comp-defgeneric-primary-cond (cdr x) stream))
              ((consp (car x))
               ...
               (format-object code2 (elt (car x) 1) nil)
               (format stream "\"))")
               (comp-defgeneric-primary-cond1 (cdr x) stream))
              (t (error* "defgeneric" x))))
    
    (defun comp-defgeneric-primary-cond1 (x stream)
        (cond ((null x) t)
              ((eq (car x) ':rest) t)
              ((eq (car x) '%rest) t)
              ((symbolp (car x)) (comp-defgeneric-primary-cond1 (cdr x) stream))
              ((consp (car x))
               (format stream " && Fmatchp(")
               (format-object code2 (conv-name (elt (car x) 0)) nil)
               (format stream ",Fmakesym(\"")
