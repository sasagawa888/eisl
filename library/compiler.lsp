    ...
    (defun comp-immediate-lambda (stream x env args tail name global test clos)
        (let* ((lam (elt x 0))
            ...
            (for ((body1 body (cdr body1)))
                 ((null body1))
                 (format stream "res=")
                 (comp stream body1 env args tail name global test clos)
                 (format stream ";"))
            (format stream "return(res);};res=")
            (comp stream (cons immediate-name val) env args tail name global test clos)
            (format stream ";res})")
